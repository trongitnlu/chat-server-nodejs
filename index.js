...
class Todo {
	constructor(id, name, priority, description, duedate) {
		this.id = id
		this.name = name
...
class Task {
	constructor(id, todoid, name, isfinished) {
		this.id = id
		this.todoid = todoid
		this.name = name
...
for (var i = 1; i <= 10; i++) {
	todos.push(new Todo(
		i,
		`Make an appointment with Mr A_${i}`,
		i,
		`Make an appointment with Mr A ${i}`,
		'2018-07-18'
	))
}
app.get('/api/todo', (req, res) => {
	res.send({ result: 'ok', data: todos });
})

app.get('/api/todo/:todoId', (req, res) => {
	res.send({ result: 'ok', data: tasks });
})

app.get('/api/task/:id', (req, res) => {
	var task = tasks.find((value) => {
		return value.todoid == req.params.id
	})
	res.send({ result: 'ok', data: task });
})
// app.get('/api/task/:id', (req, res) => {
...
for (var i = 1; i <= 10; i++) {
	tasks.push(new Task(
		i,
		i,
		`Task make an appointment with Mr A ${i}`,
		false
	))
}
...
app.put('/api/task/:id', (req, res) => {
	try {
		...
		let { name, isfinished } = req.body
		let b = tasks.find(value => value.id == id)
		b.name = name
		b.isfinished = isfinished == 1 ? true : false
		console.log(b)
		res.send({ result: 'ok' })
	}
	catch (e) {
